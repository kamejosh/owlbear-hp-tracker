...
export default class diff_match_patch {
    ...
    public Match_Distance: number;
    public Patch_DeleteThreshold: number;
    public Patch_Margin: number;
    public Match_MaxBits: number;
    ...
    private diff_bisect_(text1: string, text2: string, deadline: number): Diff[] {
        ...
        var v1 = new Array<number>(v_length);
        var v2 = new Array<number>(v_length);
    ...
    private static nonAlphaNumericRegex_ = /[^a-zA-Z0-9]/;
    private static whitespaceRegex_ = /\s/;
                        ...
                        if (count_delete !== 0 && count_insert !== 0) {
                            ...
                            if (commonlength !== 0) {
                                if (
                                    ...
                                    diffs[pointer - count_delete - count_insert - 1].operation == DIFF_EQUAL
                                ) {
                                    diffs[pointer - count_delete - count_insert - 1].text += text_insert.substring(
                                        0,
                                        commonlength
                                    );
                                } else {
     ...
     * @private
     */
    private patch_addContext_(patch: patch_obj, text: string) {
        if (text.length == 0) {
            return;
        }
        ...
        for (var x = 0; x < diffs.length; x++) {
            ...
            switch (diff_type) {
                case DIFF_INSERT:
                    ...
                    postpatch_text =
                        postpatch_text.substring(0, char_count2) + diff_text + postpatch_text.substring(char_count2);
                    break;
                case DIFF_DELETE:
